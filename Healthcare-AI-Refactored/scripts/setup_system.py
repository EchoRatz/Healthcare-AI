        ...
        if os.path.exists(path):
            try:
                ...
                if result.returncode == 0:
                    ...
                    # Update the global ollama command for other functions
                    globals()['OLLAMA_PATH'] = path
                    return True
            except Exception:
                continue
    
    # If not found, check if the service is running anyway
    print("   ❌ Ollama executable not found in PATH or common locations")
    print("   🔍 Checking if Ollama service is running...")
    
    try:
        response = requests.get("http://localhost:11434/api/version", timeout=5)
        if response.status_code == 200:
            print("   ✅ Ollama service is running (but executable not in PATH)")
            return True
...
def install_ollama_models():
    ...
    # Use the found ollama path or default to 'ollama'
    ollama_cmd = globals().get('OLLAMA_PATH', 'ollama')
