...
class HighAccuracyHealthcareQA:
        ...
        return {
            "สิทธิหลักประกันสุขภาพแห่งชาติ": {
                ...
                "excludes": [
                    ...
                    "ยาเสริม",
                    "การตรวจที่ไม่จำเป็น",
                    "การรักษาในโรงพยาบาลเอกชนที่ไม่ได้อยู่ในระบบ",
            ...
            "สิทธิบัตรทอง": {
                ...
                "excludes": ["ค่าใช้จ่าย", "30บาท", "เงินสด", "ค่าบริการ", "ค่าตรวจ"],
            ...
            "สิทธิ30บาทรักษาทุกโรค": {
                ...
                "excludes": ["ฟรี", "ไม่เสียค่าใช้จ่าย", "บัตรทอง", "ผู้สูงอายุเท่านั้น"],
                            ...
                            content_sections.append(
                                {
                                    ...
                                    "content": section,
                                    "doc_id": f"doc_{i}",
                                    "keywords": self._extract_keywords_advanced(
                                        section
                                    ),
                                }
                            )
        ...
        for section in sections:
            if len(section.strip()) > 50:
                ...
                final_sections.extend(
                    [s.strip() for s in subsections if len(s.strip()) > 50]
                )
    ...
    def search_context_semantic(
        self, question_analysis: QuestionIntent, max_sections: int = 5
    ) -> List[ContextMatch]:
    ...
    def build_optimized_prompt(
        ...
        question_analysis: QuestionIntent,
    ) -> str:
...
คำตอบ:"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                confidence = self._calculate_confidence_advanced(
                    answer_text, question_analysis, context_matches
                )
    ...
    def _extract_answers_optimized(
        self, text: str, choices: Dict[str, str]
    ) -> List[str]:
    ...
    def _calculate_confidence_advanced(
        ...
        if context_matches and any(
            match.relevance_score > 0.3 for match in context_matches
        ):
            confidence += 0.2
        ...
        if (
            "ง" in answer_text
            and context_matches
            and any(match.relevance_score > 0.4 for match in context_matches)
        ):
            confidence -= 0.1
    ...
    def validate_answer_advanced(
        ...
        policy_validation = self._validate_against_policies_advanced(
            question, choices, answers, question_analysis
        )
        if not policy_validation:
            return AnswerAnalysis(
                answers, 0.4, "Policy validation failed", False, 0.4, answers, False
            )
        ...
        return AnswerAnalysis(
            answers, 0.8, "Answer validated successfully", True, 0.8, answers, False
        )

    def _validate_against_policies_advanced(
        self,
        question: str,
        choices: Dict[str, str],
        answers: List[str],
        ...
        if question_analysis.department_terms:
            ...
            if (
                "ง" in answers
                and len(answers) == 1
                and question_analysis.primary_type != "exclusion"
            ):
                return False
    ...
    def process_questions_high_accuracy(self, test_file: str) -> List[Dict]:
        ...
        print(
            f"🚀 Processing {len(questions)} questions with high accuracy optimization..."
        )
        ...
        for i, row in enumerate(questions, 1):
            ...
            # Query LLM
            answers, confidence = self.query_llama31_optimized(
                question, choices, context_matches, question_analysis
            )

            # Validate answer
            answer_analysis = self.validate_answer_advanced(
                question, choices, answers, question_analysis
            )

            # Apply final answer
            final_answers = (
                answer_analysis.selected_answers
                if not answer_analysis.should_reject
                else answers
            )
            ...
            results.append(
                {
                    ...
                    "context_relevance": (
                        max([match.relevance_score for match in context_matches])
                        if context_matches
                        else 0.0
                    ),
                }
            ...
            if i % 25 == 0:
                ...
                print(
                    f"  📊 {i}/{len(questions)} ({i/len(questions)*100:.1f}%) | Rate: {rate:.1f} q/s | ETA: {eta/60:.1f}min"
                )
...
async def main():
    ...
    if results:
        ...
        avg_context_relevance = (
            sum(r["context_relevance"] for r in results) / total_questions
        )

        print(f"\n📊 DETAILED SUMMARY:")
        print(f"  Total questions: {total_questions}")
        print(
            f"  High confidence answers: {high_confidence} ({high_confidence/total_questions*100:.1f}%)"
        )
        print(
            f"  Validation passed: {validation_passed} ({validation_passed/total_questions*100:.1f}%)"
        )
