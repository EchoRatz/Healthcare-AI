...
class UltraFastQA:
    ...
    def query_llama31_with_context(
...
คำตอบ:"""
            ...
            if section_match:
                ...
                if len(section_chars) > 1:
                    ...
                    if 2 <= len(unique_answers) <= 4:
                        ...
                        if not any(
                            word in answer_section
                            for word in ["คือ", "เป็น", "คำตอบ", "ข้อ"]
                        ):
    ...
    def process_ultra_fast(self, test_file: str) -> List[Dict]:
        ...
        try:
            ...
            for i, row in enumerate(questions, 1):
                ...
                predicted_answers, confidence = self.query_llama31_with_context(
                    question, choices
                )

                # Enhanced logical validation (always applied)
                validation_result = LOGICAL_VALIDATOR.validate_answer(
                    question, choices, predicted_answers
                )
                if validation_result.corrections_made:
                    print(
                        f"    🔧 Logic Fix: {validation_result.original_answer} → {validation_result.validated_answer}"
                    )
                ...
                if MCP_AVAILABLE and (
                    ...
                    try:
                        validated_answers, mcp_confidence, validation_source = (
                            validate_answer_sync(question, predicted_answers, choices)
                        )

                        if (
                            "CORRECTED_CONTRADICTION" in validation_source
                            or "OVERRIDE" in validation_source
                        ):
                ...
                if i <= 3:
                    print(
                        f"      Q{i}: {question[:35]}... → {predicted_answers} (conf: {confidence:.2f})"
                    )
    ...
    def process_ultra_fast_string(self, questions: str) -> List[Dict]:
        """
        Process a string of questions directly.
        """
        results = []
        try:
            # Split the string into individual questions (assuming newline-separated)
            question_list = questions.split("\n")
            for question in question_list:
                if question.strip():
                    parsed_question, choices = self.parse_question(question)
                    if parsed_question and choices:
                        answers, confidence = self.query_llama31_with_context(
            ...
            raise RuntimeError(f"Error processing questions: {str(e)}")
    ...
    if results:
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, "validate_format.py", "ultra_fast_submission.csv"],
                capture_output=True,
                text=True,
                timeout=10,
            )
